...
// Final Class
final class UmeetEvent {

	// Theme Version
	private $umeetevent_version = '1.0';
	...
	private function setup() {
		...
		$enqueu = new UmeetEvent_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->umeetevent_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'umeetevent_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'umeetevent', UMEET_EVENT_DIR_PATH . '/languages' );
		...
		add_image_size( 'umeetevent_widget_post_thumb', 80, 80, true ); 

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'umeetevent-np-thumb', 60, 60, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'umeetevent' ),
            'social-menu'   => esc_html__( 'Social Menu', 'umeetevent' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = UMEET_EVENT_DIR_CSS_URI;
		$jsPath  = UMEET_EVENT_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					'handler'		=> 'umeetevent-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->umeetevent_version,
				),
				array(
					'handler'		=> 'umeetevent-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'umeetevent' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate umeetevent theme customizer
		$umeetevent_theme_customizer = new umeetevent_theme_customizer();
	}
	...
	public function epsilon_quickie() {
			...
			'links' => array(
				array(
					'link_to'   => 'umeetevent_options_panel',
	...
	public function set_elementor_flag() {
		update_option( 'umeetevent_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'umeetevent-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'umeetevent_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'umeetevent-elementor-default-style',  UMEET_EVENT_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->umeetevent_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'umeetevent_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'umeetevent-elementor-notice', UMEET_EVENT_DIR_JS_URI.'umeetevent-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'umeetevent-elementor-notice',
				'umeeteventElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'umeetevent-elementor-notice-nonce' ),
				)
			);
		}
